...
/*                                                                                    */
/*  Copyright © 2012-2014 by J.Neuhoff (www.mhccorp.com)                              */
/*                                                                                    */
...
final class Factory 
{
	...
	private function newInstance( $type, $file, $class, $args=array() ) {
		...
		switch ($type) {
			...
			case self::MODEL_CLASS:
				...
				break;
			case self::EVENT_CLASS:
				$i = strrpos( $file, '/event/' );
				if ($i===FALSE) {
					trigger_error("Invalid file path '$file' for event class '$class'");
					exit;
				}
				break;
	...
	public function newEvent( $path ) {
		// find the original class file
		$file = DIR_APPLICATION . 'event/' . $path . '.php';
		if (file_exists( $file ) && is_file( $file )) {
			$class = $this->pathToClassName( 'Event', $path );
		} else {
			trigger_error("Cannot find event class file for path '$path'");
			exit;
		}

		// get a new instance of the class
		return $this->newInstance( self::EVENT_CLASS, $file, $class, array( $this->registry ) );
	}
