...
class ControllerModulePavblog extends Controller {
	...
	public function blog(){
		...
		if( isset($this->request->get['id']) ){
			...
			if( $blog['image_landscape'] ){
				$blog['thumb_landscape'] = $this->model_tool_image->resize($blog['image_landscape'], 180, 180);
			}
			if( $blog['image_portrait'] ){
				$blog['thumb_portrait'] = $this->model_tool_image->resize($blog['image_portrait'], 180, 180);
			}
		}
	...
	public function moduleBlogLatest(){
		...
		// prefixclass_lat
		if (isset($this->request->post['pavbloglatest_module']['useimage'])) {
			$this->mdata['useimage'] = $this->request->post['pavbloglatest_module']['useimage'];
		} elseif (!empty($module_info)) {

			$this->mdata['useimage'] = $module_info['useimage'];
		} else {
			$this->mdata['useimage'] = "lanscape";
		}
